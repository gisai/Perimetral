        ...
        if [ "$capturing" ]; then
            if [[ $line =~ ^[0-9a-fA-F]{2}\ [0-9a-fA-F] ]]; then
                ...
                if [[ $packet =~ ^04\ 3E\ 2A\ 02\ 01\ .{26}\ 02\ 01 ]]; then
					...
					NOW=$(date +%s)
                    if [[ $2 == "-b" ]]; then
	                    echo "{tech: $tech, ID: {IDSystem:$UUID, IDGroup:$MAJOR, IDPerson: $MINOR}, IDAnte:$idAntena, pow:$POWER, rssi:$RSSI, time:$NOW}"
                    else
                        # Mandamos la salida para la cola fifo
    	                echo "{tech: $tech, ID: {IDSystem:$UUID, IDGroup:$MAJOR, IDPerson: $MINOR}, IDAnte:$idAntena, pow:$POWER, rssi:$RSSI, time:$NOW}" > $2
                    fi
                fi
                capturing=""
                packet=""
