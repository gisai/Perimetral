...
class NodeDiscoveryPersonal {
	 ...
	 * @see XBeeDevice
	 */
	public NodeDiscoveryPersonal(XBeeDevice xbeeDevice) {
		if (xbeeDevice == null)
			...
			if (!is802Compatible) {				
				...
				while (discovering) {
					if (System.currentTimeMillis() < deadLine)
						...
						deviceList.clear();
						sendNodeDiscoverCommand(id);
						deadLine = System.currentTimeMillis()+calculateTimeout(listeners);
					}
				}
			} else {
	...
	private void notifyDiscoveryFinished(List<IDiscoveryListener> listeners, String error) {
		...
		System.out.println("Entro en final");
		for (IDiscoveryListener listener : listeners)
